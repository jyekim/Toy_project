    ...
    class AudioPlayer extends HTMLElement {
      ...
      updateFrequency() {
        ...
        for (let i = 0; i < barCount; i++) {
          ...
          // iPerc는 루프 반복 횟수를 주파수 데이터의 퍼센트로 변환하는 변수이다. 즉 루프 반복이 진행되는 동안 주파수 데이터의위치 계산하는데 사용
          const pos = Math.round((bufferSize * iPerc) / 100);
          const frequency = this.dataArray[pos];
          //pos는 해당 반복에서 주파수 데이터 실제 위치를 계산함  이 위치에 해당하는 주파수데이터 얻어옴
          const frequencyPerc = (frequency * 100) / 255;
          // 얻어온 주파수 값을 퍼센트로 변환하는 값 / 그래픽 표현의 높이를 결정 
          const barHeight = (frequencyPerc * this.canvas.height) / 100;
          // 막대 높이를 결정함 
          const y = this.canvas.height - barHeight;
          // 막대의 시작 위치(캔버스의 하단) 결정함
          this.canvasCtx.fillStyle = `rgba(${frequency}, 255, 100)`;
          this.canvasCtx.fillRect(x, y, this.barWidth, barHeight);
          //막대를 실제로 그리는 부분으로 이전에 계산한 위치와 높이를 사용하여 캔버스에 막대를 그림
          x += (this.barWidth + this.barGap);
        }
        
        // 브라우저에게 애니메이션을 업데이트 할 수 있는 기회를 제공함 이 함수가 주파수 데이터를 지속적으로 업데이트하여 실시간 오디오 시각화 생성
      ...
      render() {
        ...
        //<canvas>요소에서 2D 랜더링 컨텍스트 가져와서 this.canvasCtx에 할당함
        // support retina display on canvas for a more crispy/HD look
        const scale = window.devicePixelRatio; //현재 디바이스의 픽셀 비율 가져옴 
        ...
        //<figuration>요소에 오디오소스의 제목 설정함. 오디오 소스가 제공되었는지 확인후 제목 있다면 해당 값 표시 없으면 NO Audio 어쩌구 표시 
